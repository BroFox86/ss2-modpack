/*
	...
	--------------------------------------------------------------------------------
	Item Location Randomizer (WIP)
	Enabled by placing a file named "modItemRand" in the "scriptdata" folder.
	May be enabled/disabled at any time in a game, but already-moved items
	will not be returned to their original locations when disabling.
	WARNING: Not implemented in any levels. Not multiplayer-safe.

	Shuffles the placement of important items among a set of predefined locations.
	================================================================================
...
// debugging modes
const DEBUG_ECO_LEVEL = 0; // trace eco/spawn activity (0=none, 1=basic, 2=verbose)
const DEBUG_ECO_TYPES = false; // validate ecology/spawner eco types matching
const DEBUG_TRIPWIRES = false; // visualize and validate (most) tripwires in-game
const DEBUG_MODIFY    = false; // trace all weapon modify activity
const DEBUG_QUESTS    = false; // trace all quest variable changes
const DEBUG_PDA       = false; // enable log/note/email tabs for all decks in PDA
...
// various behaviors
const TOXIN_DECAY_RATE       = 120; // interval for reducing toxin level, in seconds (0=disable)
const WORM_PILE_RESPAWN_TIME = 300; // delay before destroyed worm piles respawn, in seconds
const BASE_GRENADE_SCALE     = 1.85; // base grenade damage source scale (must match gamesys value)
const COMPASS_OFFSET         = 90; // global compass offset, in degrees
const QBR_ANTI_SPAWNCAMP     = true; // move AIs away from QBR when respawning
const KEYPAD_STATE_LIGHT     = true; // display locked/unlocked light on keypads
const MIDGAME_ACT_CHECK      = true; // check for activating SCP in middle of non-SCP game
const DROMED_GOODIES         = true; // give extra resources/stats in editor mode
...
class scpPlayerScript extends SqRootScript {
	function OnBeginScript() {
		...
		initTraits();
		if (DEBUG_QUESTS && IsEditor()) {
			Quest.SubscribeMsg(self, "*");
		}

		if (DEBUG_PDA && IsEditor()) {
			// this doesn't work when executed immediately
			PostMessage(self, "DebugPDA");
		}

		if (DROMED_GOODIES && IsEditor()) {
			// this doesn't work when executed immediately
			PostMessage(self, "DromedGoodies");
		}
	...
	function OnEndScript() {
		...
		if (!IsEditor()) {
			Quest.Set("MapID", scp.EncMapName(scp.MapName()), eQuestDataType.kQuestDataCampaign);
		}
	}
	...
	// give some stuff in editor mode to make testing a little easier
	function OnDromedGoodies() {
		// don't give stuff when starting from a saved game
		if (ShockGame.SimTime() > 10) {
			return;
		}
		// make it rain
		ShockGame.PayNanites(-500);
		ShockGame.AddExp("Player", 500, false);
		ShockGame.AddInvObj(Object.Create("Wrench"));
		Property.Set("Player", "BaseWeaponDesc", "Conventional", 1);
		Property.Set("Player", "BaseTechDesc", "Hack", 1);
	}
	...
	function OnSaveOverlays() {
		...
		foreach (slot in slotList) {
			...
			if (item && Property.Possessed(item, "Energy")) {
				charge = Property.Get(item, "Energy");
				if (charge > 0) {
					SetData("OrigCharge" + slot, charge);
	...
	function OnRestoreOverlays() {
		...
		foreach (slot in slotList) {
			item = ShockGame.Equipped(slot);
			if (item && Property.Possessed(item, "Energy") && IsDataSet("OrigCharge" + slot)) {
				Property.SetSimple(item, "Energy", GetData("OrigCharge" + slot));
				ClearData("OrigCharge" + slot);
			}
		}
	...
	// slowly decay toxin poisoning
	// Toxin damage is dealt to player by PlayerScript with no type, so it's impossible
	// to differentiate between toxin and radiation damage events. Since radiation
	// damage has a different, faster tick interval than toxin damage (6 seconds vs 10),
	// toxins will decay over twice as fast when taking radiation damage. This could be
	// fixed by not allowing toxins to decay faster than once every ten seconds, but the
	// added complexity isn't necessary since this isn't really exploitable.
	function OnDamage() {
		if (message().kind != 0 || message().culprit != self || TOXIN_DECAY_RATE == 0) {
			return;
		}
		local toxin = GetProperty("Toxin");
		if (!toxin) {
			return;
		}
		local decay = GetData("ToxinDecay") || 0;
		if (toxin > (GetData("ToxinLast") || 0)) {
			decay = 0;
		}
		else if (decay < (TOXIN_DECAY_RATE / 10)) {
			decay++;
		}
		else {
			decay = 0;
			toxin--;
			SetProperty("Toxin", toxin);
			if (toxin <= 0) {
				toxin = 0;
				ShockGame.OverlayChange(kOverlayPoison, kOverlayModeOff);
			}
		}
		SetData("ToxinDecay", decay);
		SetData("ToxinLast", toxin);
	}
	...
	// prevent enemies from spawn-camping player (except in multiplayer)
	function clearQBR() {
		if (
			QBR_ANTI_SPAWNCAMP &&
			ShockGame.AllowDeath() &&
			Quest.Get("AllowRespawn") &&
			scp.PlayerNanites() >= 10 &&
			Quest.Get("Difficulty") != 5
		) {
			PostMessage(scp.GetConcretes("Res_Station_Button")[0], "PlayerDead");
		}
	}

	// implement unused death taunts
	function deathTaunts() {
		local deathMode;
	...
	// check if player has transitioned to a new map
	// halt and floor player if so
	function checkNewMap() {
		local curMap = scp.MapName();
		local oldMap = scp.DecMapName(Quest.Get("MapID"));
		// some map transitions need to preserve player momentum and/or height above floor
		if (!Physics.HasPhysics(self) ||
			(curMap == oldMap) ||
			(oldMap == "station") ||
			(oldMap == "many") ||
			(curMap == "hydro2" && Quest.Get("ElevState") != 2) ||
			(curMap == "rick1" && oldMap == "command1") ||
			(curMap == "command1" && oldMap == "rick1") ||
			(curMap == "rick2" && oldMap == "rick1")
		) {
			return;
		}
		...
		local hitLoc = vector();
		if (Engine.PortalRaycast(pos, pos + vector(0, 0, -8), hitLoc)) {
			SetProperty("Position", "Location", vector(pos.x, pos.y, hitLoc.z + 3.0));
		}
	}
...
// --------------------------------------------------------------------------------
// Fixes Localized Pyrokinesis triggering unnecessary grunts when burning objects
...
// --------------------------------------------------------------------------------
// Fixes Remote Circuitry Manipulation allowing hacking of normally non-hackable objects
class CyberPsi extends RootPsi {
	function ActivatePsi() {
		...
		else if (scp.IsHackable(obj)) {
			ShockGame.HRM(0, obj, true);
		}
		else {
...
// --------------------------------------------------------------------------------
// Enhanced Soma Drain script
// - Fixes Soma Drain allowing draining targets for more HP than they have.
// - Fixes drain FX not appearing on non-creature targets.
// - Improves detection of valid targets.
// (on all vulnerability metaprops, remove or disable DrainStim that heals player)
class SomaDrain extends RootPsi {
	...
	function isDrainable(obj) {
		return Link.AnyExist("StimSensor", obj, "DrainStim") &&
		!Object.InheritsFrom(obj, "Undrainable") &&
		!Object.InheritsFrom(obj, "Invulnerable") &&
		!Object.InheritsFrom(obj, "InvulnManyBrain") &&
		(!Link.AnyExist("Receptron", obj, "DrainStim") || LinkTools.LinkGetData(Link.GetOne("Receptron", obj, "DrainStim"), "Reaction") != 0);
	}
...
class scpSecurityComputer extends SqRootScript {
	...
	function hackSecurity(hackTime) {
		...
		local kill = [];
		local linkFlavors = ["~AIAwareness", "~AIAttack", "~AITarget", "~AIInvest"];
...
// - Supports unconditional reset via ResetEx message.
// - Fixes Electron Suppression not blinding cameras
...
class scpCameraHelper extends SqRootScript {
	function OnBeginScript() {
		// sanity check
		if (IsDataSet("Dead") || !Object.Exists(self) || !Physics.HasPhysics(self)) {
			return;
		}
		...
		if (scp.IsModEnabled("modSecurity") && !(map == "earth" || map == "shodan")) {
			...
			if (GetProperty("ObjState") != eObjState.kObjStateHacked) {
				...
				SetProperty("HackDiff", "Cost", scp.Round(hackCost / 2));
			}
		}
	...
	// force alertness reset regardless of alarm state
	function OnResetEx() {
		AI.SetMinimumAlert(self, eAIScriptAlertLevel.kNoAlert);
...
class scpTurret extends SqRootScript {
	...
	function OnFrobWorldEnd() {
		if (GetProperty("ObjState") == eObjState.kObjStateBroken) {
			if (ShockGame.OverlayOn(kOverlayContainer)) {
...
// (apply to only the Monsters (-162) archetype)
...
// - Saves AI's start location, for use when relocating away from QBR.
// - Adds listener to teleport self back to start location.
class scpBaseAI extends SqRootScript {
	function OnBeginScript() {
		PostMessage(self, "StartPos");
	}
	...
	function OnStartPos() {
		if (Object.Exists(self) && (GetProperty("HitPoints") > 0)  && !IsDataSet("StartPos") && !Object.HasMetaProperty(self, "Start Hidden")) {
			SetData("StartPos", Object.Position(self));
		}
	}

	function OnGoHome() {
		if (IsDataSet("StartPos") && Object.Exists(self) && (GetProperty("HitPoints") > 0)) {
			Object.Teleport(self, GetData("StartPos"), vector());
		}
	}
}
...
class scpBaseEgg extends SqRootScript {
	...
	function emitOrgan(type) {
		...
		if (!Quest.Exists("First " + type)) {
			chance = 1;
			Quest.Set("First " + type, 1, eQuestDataType.kQuestDataCampaign);
		}
		if (scp.Rand() < chance) {
			local obj = scp.CreateAt(type, self);
			Property.Set(obj, "PhysState", "Velocity", vector(scp.Rand(10) - 5, scp.Rand(10) - 5, scp.Rand(3) - 1));
			Property.Set(obj, "PhysState", "Rot Velocity", vector(scp.Rand(10) - 5, scp.Rand(10) - 5, scp.Rand(5) + 1));
		}
	}
}
...
// --------------------------------------------------------------------------------
// Remove duplicate organs from monster loot
class scpOrganDedupe extends SqRootScript {
	function OnSlain() {
		// wait until loot population has occurred
		PostMessage(self, "ScanLoot");
	}
	...
	function OnScanLoot() {
		...
		local archFound = -1;
		local destroy = [];
		foreach (lnk in Link.GetAll("Contains", self)) {
			obj = LinkDest(lnk);
			arch = Object.Archetype(obj);
			if (Object.InheritsFrom(arch, "Researchable")) {
				if (arch == archFound) {
					destroy.push(obj);
				}
				else {
...
// --------------------------------------------------------------------------------
// Enhanced transient corpse script
// - Increases delay before corpse removal attempted from 1 to 2 minutes.
// - Does not attempt corpse removal when player is nearby or looking.
class scpTransientCorpse extends SqRootScript {
	function OnSlain() {
		if (!isPlotCritical()) {
			SetOneShotTimer("Decay", 120);
		}
	}

	function OnTimer() {
		if (message().name == "Decay") {
			local dist = scp.DistSqFlat(Object.Position(self), Object.Position("Player"));
			if ((dist > 3000) && (!Object.RenderedThisFrame(self))) {
				removeCorpse();
			}
			else {
				SetOneShotTimer("Decay", 30);
			}
		}
	}
	...
	function removeCorpse() {
		local id;
		local contains = [];
		foreach (id in Link.GetAll("Contains", self)) {
			contains.push(LinkDest(id));
		}
		foreach (id in contains) {
			Object.Destroy(id);
		}
		Object.Destroy(self);
	}

	function isPlotCritical() {
		local lnk;
		foreach (lnk in Link.GetAll("Contains", self)) {
			if (Property.Possessed(LinkDest(lnk), "PlotCritical")) {
				return true;
			}
		}
		return Property.Possessed(self, "PlotCritical");
	}
}

// --------------------------------------------------------------------------------
// Fixes AI responses failing to execute if player saves and reloads an autosave
// (assumes that immobile AI has not received its signal yet)
class scpFixAIRes extends SqRootScript {
	function OnBeginScript() {
		local ai;
		local pos = Object.Position(self);
		if (!IsDataSet("IdleX")) {
		...
		else if (fabs(pos.x - GetData("IdleX")) < 0.25 && fabs(pos.y - GetData("IdleY")) < 0.25) {
			ai = GetProperty("AI");
			SetProperty("AI", "None");
			SetProperty("AI", ai);
		}
...
class scpElevatorHelper extends SqRootScript {
	...
	// determine whether we're part of a keypad door setup
	// and if the keypad has been unlocked
	function keypadOpen() {
		local lnk, obj;
		local door = LinkDest(Link.GetOne("SwitchLink", self));
		if (Property.Possessed(door, "RotDoor")) {
			return false;
		}
		foreach (lnk in Link.GetAll("~SwitchLink", door)) {
			obj = LinkDest(lnk);
			if (Object.InheritsFrom(obj, "Keypad") && Property.Get(obj, "ObjState") == eObjState.kObjStateHacked) {
				return true;
			}
		}
		return false;
...
// --------------------------------------------------------------------------------
// Keypad helper script
// - Plays success/failure sound effects
// - Passes TurnOn/TurnOff messages to any linked doors
// - Switches models to display current lock state
// - Deactivates any overlays when hacked with ICE pick
// - Displays optional locked message:
...
class scpKeypadHelper extends SqRootScript {
	...
	function doorControl(msg) {
		local lnk, obj;
		if (IsDataSet("Opened") || msg == "TurnOff") {
			foreach (lnk in Link.GetAll("SwitchLink", self)) {
				obj = LinkDest(lnk);
				if (Object.InheritsFrom(obj, "Doors")) {
					PostMessage(obj, msg);
				}
			}
		}
	...
	function setModel() {
		if (!KEYPAD_STATE_LIGHT) {
			return;
		}
		local model = GetProperty("ModelName");
		if (model == "keypad" || model == "keypadg" || model == "keypadr") {
			SetProperty("ModelName", IsDataSet("Opened") ? "keypadg" : "keypadr");
		}
	}
}

// --------------------------------------------------------------------------------
// Turns all card slots for a door green when any card slot used.
// Assumes all card slots linked directly to door(s), and no more than two card
// slots per door setup.
class scpCardSlotHelper extends SqRootScript {
	function OnNowUnlocked() {
		PostMessage(self, "UnlockOthers");
	}

	function OnUnlockOthers() {
		local obj, lnk, lnk2;
		foreach (lnk in Link.GetAll("SwitchLink", self)) {
			foreach (lnk2 in Link.GetAll("~SwitchLink", LinkDest(lnk))) {
				obj = LinkDest(lnk2);
				if (obj != self && Object.InheritsFrom(obj, "Card slot") && Property.Get(obj, "Locked")) {
					Property.SetSimple(obj, "Locked", false);
					return;
				}
			}
		}
...
class scpReplicatorHelper extends SqRootScript {
	...
	function OnTimer() {
		...
		if (mName == "StartSpawn") {
			...
			// abuse detail attachments to determine relative position/rotation of bumper
...
class scpGlass extends SqRootScript {
	function OnSlain() {
		...
		// ensure windows with door property added to block audio stop doing so when destroyed
		if (HasProperty("TransDoor")) {
...
// --------------------------------------------------------------------------------
// Recharging station helper script
// - Animates lights when station used
// - Closes upgrade overlays so player can't cheat
class scpEnergyStationHelper extends SqRootScript {
	function OnFrobWorldEnd() {
		scp.UpgradeCheatStopper();
		SetData("BlinkCount", 20);
		lightsOn();
		setPartColor(38);
		blinkLights();
	}
	...
	function blinkLights() {
		local count = GetData("BlinkCount") - 1;
		SetData("BlinkCount", count);
		if (count > 0) {
			SetData("LightTimer", SetOneShotTimer("Lights", 0.09));
			SetProperty("SelfIllum", 1 - GetProperty("SelfIllum"));
		}
		else {
			lightsOn();
		}
	...
	function lightsOn() {
		if (IsDataSet("LightTimer")) {
			SetProperty("SelfIllum", 1);
			setPartColor(87);
			KillTimer(GetData("LightTimer"));
			ClearData("LightTimer");
		}
	}

	function setPartColor(n) {
		local obj = LinkDest(Link.GetOne("~ParticleAttachement", self, 0));
		Property.Set(obj, "ParticleGroup", "color (palettized)", n);
	}
}

// --------------------------------------------------------------------------------
// Quantum entanglement scanner helper script
// - Makes QBR scanner pads no longer highlight after use.
// - Moves hostile AIs away from QBR when player respawns.
// - Breaks all AI links to player when player respawns.
class scpResurrectMachineHelper extends SqRootScript {
	...
	function OnPlayerDead() {
		// player script waits 5 seconds after death to respawn,
		// so don't do anything until just before that
		SetOneShotTimer("PlayerDead", 4.9);
	}

	function OnTimer() {
		if (message().name != "PlayerDead") {
			return;
		}

		// teleport AIs near QBR away from it
		local obj, lnk, flavor, dest, destList;
		local qbrPos = Object.Position(self);
		foreach (obj in scp.GetConcretes("Monsters", aiFilter, self)) {
			// build list of ALL possible teleport destinations
			destList = [];
			// marker spawned from
			if (Link.AnyExist("~Spawned", obj)) {
				destList.push(LinkDest(Link.GetOne("~Spawned", obj)));
			}
			// all ecology spawn points
			if (Property.PossessedSimple(obj, "EcoType")) {
				destList.extend(spawnPoints(obj));
			}
			// all linked patrol points
			if (Link.AnyExist("AICurrentPatrol", obj)) {
				destList.extend(patrolPoints(obj));
			}
			// remove nearby teleport destinations
			destList = destList.filter(function(ndx, obj){return scp.DistanceSq(qbrPos, Object.Position(obj)) > 2000})
			if (destList.len() > 0) {
				// teleport to random destination
				dest = destList[Data.RandInt(0, destList.len() - 1)];
				Object.Teleport(obj, vector(), vector(), dest);
			}
			else {
				PostMessage(obj, "GoHome");
			}
		}

		// break all AI links to player, since player has a new body now
		foreach (flavor in ["~AIAwareness", "~AIAttack", "~AITarget", "~AIInvest"]) {
			Link.DestroyMany(flavor, "Player", 0);
		}
	}

	// filter for AIs that are alive, hostile, mobile, aware of the player, and close on the horizontal plane
	function aiFilter(obj, self) {
		return(
			Property.Get(obj, "HitPoints") > 0 &&
			scp.DistSqFlat(Object.Position(self), Object.Position(obj)) < 1000 &&
			fabs(Object.Position(self).z - Object.Position(obj).z) < 8 &&
			(
				Object.InheritsFrom(obj, "Hybrids") ||
				Object.InheritsFrom(obj, "Cyborgs") ||
				Object.InheritsFrom(obj, "Arachnid") ||
				Object.InheritsFrom(obj, "Robots") ||
				Object.InheritsFrom(obj, "Grub") ||
				Object.InheritsFrom(obj, "Swarm") ||
				Object.InheritsFrom(obj, "SHODAN")
			) &&
			!Object.HasMetaProperty(obj, "Start Hidden") &&
			!Object.HasMetaProperty(obj, "Docile") &&
			!Object.HasMetaProperty(obj, "Blind") &&
			!Object.HasMetaProperty(obj, "Deactivated") &&
			!Object.HasMetaProperty(obj, "Good Guy")
		);
	}

	// find all spawn points linked from AI's ecology
	function spawnPoints(obj) {
		local eco, spawner, lnk;
		local spawnList = [];
		local ecoList = scp.GetConcretes("Ecology", @(obj, eco) Property.Get(obj, "EcoType") == eco, Property.Get(obj, "EcoType"));
		if (ecoList.len() == 0) {
			return [];
		}
		eco = ecoList[0];
		if (!Link.AnyExist("SwitchLink", eco)) {
			return [];
		}
		spawner = LinkDest(Link.GetOne("SwitchLink", eco));
		if (Property.Get(spawner, "Spawn", "Flags") & eSpawnFlags.kSpawnFlagSelfMarker) {
			spawnList.push(spawner);
		}
		foreach (lnk in Link.GetAll("SpawnPoint", spawner)) {
			spawnList.push(LinkDest(lnk));
		}
		return spawnList;
	}

	// find all patrol points accessible from current patrol point
	function patrolPoints(obj) {
		local curPt, lnk, lnkD, pathList;
		local visitedList = [];
		local toVisitList = [LinkDest(Link.GetOne("AICurrentPatrol", obj))];
		while(toVisitList.len()) {
			curPt = toVisitList.pop();
			if (visitedList.find(curPt) != null) {
				continue;
			}
			visitedList.push(curPt);
			// fetch all incoming/outgoing patrol links
			pathList = [];
			foreach (lnk in Link.GetAll("AIPatrol", curPt)) {
				pathList.push(lnk);
			}
			foreach (lnk in Link.GetAll("~AIPatrol", curPt)) {
				pathList.push(lnk);
			}
			// stash unvisited patrol points
			foreach (lnk in pathList) {
				lnkD = LinkDest(lnk);
				if (visitedList.find(lnkD) == null) {
					toVisitList.push(lnkD);
				}
			}
		}
		return visitedList;
...
class scpLabAssHelper extends SqRootScript {
	function OnTurnOff() {
...
// --------------------------------------------------------------------------------
// Fixes GamePig MFD not closing when GamePig moved in inventory or dropped.
class scpGameBoyHelper extends SqRootScript {
	function OnContained() {
		if (ShockGame.OverlayGetObj() == self) {
			ShockGame.OverlayChangeObj(kOverlayMFDGame, kOverlayModeOff, self);
		}
	}
}
...
// - Displays name of game cartridge installed.
...
class scpMiniGameCart extends SqRootScript {
	...
	function installCart() {
		...
		scp.AddText("MFDGameCartEx", "misc", "%s game cartridge installed.", Data.GetObjString(self, "objshort"));
	}
}
...
// - Fixes being thrown into world if dragged onto non-hackable world object.
// - Sends message to hacked object letting them know they were hacked by an ICE-Pick.
...
class scpFreeHack extends SqRootScript {
	function OnFrobToolEnd() {
		local obj = message().DstObjId;
		local objState = Property.Get(obj, "ObjState");
		if (scp.IsHackable(obj)) {
			ShockGame.PreventSwap();
			// check for already hacked/opened
			if ((objState == eObjState.kObjStateHacked) || (Object.InheritsFrom(obj, "Hackable Crate") && objState == eObjState.kObjStateNormal)) {
				scp.AddText("FreeHackHacked", "misc");
			}
			else if (objState != eObjState.kObjStateBroken) {
				Networking.SendToProxy("Player", obj, "HackSuccess", null);
				Networking.SendToProxy("Player", obj, "ICEPick", null);
				scp.AddText("FreeHack", "misc");
				scp.Consume(self, "hack_success");
				// hide hack plug overlay
				if (ShockGame.OverlayGetObj() == obj) {
					ShockGame.OverlayChangeObj(kOverlayHRMPlug, kOverlayModeOff, obj);
				}
			}
		}
		else {
			// error if used on world object, otherwise will swap in inventory
			if (!scp.IsContained(obj)) {
				ShockGame.PreventSwap();
				scp.AddText("FreeHackCant", "misc");
			}
		}
...
// --------------------------------------------------------------------------------
// Enhanced auto-repair unit script
// - Plays hack success sound when used.
// - Fixes item MFD not being properly reinitialized after repair.
// - Fixes being thrown into world if dragged onto non-repairable world object.
class scpFreeRepair extends SqRootScript {
	function OnFrobToolEnd() {
		local obj = message().DstObjId;
		if (Property.Get(obj, "ObjState") == eObjState.kObjStateBroken) {
			ShockGame.PreventSwap();
			ShockGame.SetObjState(obj, eObjState.kObjStateNormal);
			scp.AddText("FreeRepair", "misc");
			scp.Consume(self, "hack_success");
			// reset MFD
			if (ShockGame.OverlayGetObj() == obj) {
				local o;
				local overlays = [kOverlayHRM, kOverlaySetting, kOverlaySecurComp, kOverlayRep, kOverlayKeypad, kOverlayTurret, kOverlayContainer];
				foreach (o in overlays) {
					if (ShockGame.OverlayOn(o)) {
						ShockGame.OverlayChangeObj(o, kOverlayModeOff, obj);
						break;
					}
				}
				PostMessage(obj, scp.IsContained(obj) ? "FrobInvEnd" : "FrobWorldEnd");
			}
		}
		// error if used on world object, otherwise will swap in inventory
		else if (!scp.IsContained(obj)) {
			ShockGame.PreventSwap();
			scp.AddText("FreeRepairCant", "misc", "Item cannot be repaired.");
		}
	}

	function OnFrobInvEnd() {
		scp.AddText("HelpFreeRepair", "misc");
	}
}

// --------------------------------------------------------------------------------
// Enhanced French-Epstein device script
// - Plays hack success sound when used.
// - Fixes item MFD not being properly reinitialized after modification.
// - Fixes being thrown into world if dragged onto non-moddable world object.
class scpFreeModify extends SqRootScript {
	function OnFrobToolEnd() {
		local obj = message().DstObjId;
		if (ShockGame.ValidGun(obj)) {
			ShockGame.PreventSwap();
			local modLevel = Property.Get(obj, "GunState", "Modification");
			if (modLevel < 2) {
				ShockGame.SetModify(obj, modLevel + 1);
				scp.AddText("FreeModify", "misc");
				scp.Consume(self, "hack_success");
				// reset MFD
				if (ShockGame.OverlayGetObj() == obj) {
					if (ShockGame.OverlayOn(kOverlaySetting)) {
						ShockGame.OverlayChangeObj(kOverlaySetting, kOverlayModeOff, obj);
					}
					else if (ShockGame.OverlayOn(kOverlayHRM)) {
						ShockGame.OverlayChangeObj(kOverlayHRM, kOverlayModeOff, obj);
					}
					PostMessage(obj, "FrobInvEnd");
				}
			}
			else {
				scp.AddText("FreeModifyMax", "misc");
			}
		}
		// error if used on world object, otherwise will swap in inventory
		else if (!scp.IsContained(obj)) {
			ShockGame.PreventSwap();
			scp.AddText("FreeModifyCant", "misc");
		}
	}

	function OnFrobInvEnd() {
		scp.AddText("HelpFreeModify", "misc");
	}
}

// --------------------------------------------------------------------------------
// Enhanced maintenance tool script
...
class scpWrench extends SqRootScript {
	function OnFrobToolEnd() {
		...
		else if (!scp.IsContained(fixObj)) {
			ShockGame.PreventSwap();
...
// --------------------------------------------------------------------------------
// Enhanced recycler script
// - Adds messages for item recycled and nanites gained, attempt to recycle invalid item.
// - Allows swapping with non-recyclable items in inventory.
// - Fixes being thrown into world if dragged onto non-recyclable world object.
// - Fixes bug with recycling equipped implants.
class scpRecycler extends SqRootScript {
	function OnFrobToolEnd() {
		local nanites, objName;
		local obj = message().DstObjId;
		local recVal = Property.Possessed(obj, "Recycle") ? Property.Get(obj, "Recycle") : 0;
		if (recVal > 0) {
			if (scp.IsEquipped(obj)) {
				scp.AddText("RecyclerEquip", "misc", "Item cannot be recycled while equipped.");
			}
			else {
				nanites = recVal * (Property.Get(obj, "StackCount") || 1);
				objName = Property.Get(obj, "ObjState") == eObjState.kObjStateUnresearched ? Data.GetString("research", "NameUnresearched") : Data.GetObjString(obj, "objshort");
				ShockGame.PreventSwap();
				ShockGame.PayNanites(-nanites);
				if (nanites == 1) {
					scp.AddText("RecyclerSuccess1", "misc", "%s recycled into 1 nanite.", objName);
				}
				else {
					scp.AddText("RecyclerSuccess", "misc", "%s recycled into %d nanites.", objName, nanites);
				}
				if (scp.IsContained(obj)) {
		...
		// error if used on world object, otherwise will swap in inventory
			...
			ShockGame.PreventSwap();
			scp.AddText("RecyclerCant", "misc", "Item cannot be recycled.");
		}
	}

	function OnFrobInvEnd() {
		scp.AddText("HelpRecycler", "misc");
	}
}

// --------------------------------------------------------------------------------
// Enhanced battery script
// - Adds messages for item already charged, attempt to charge invalid item.
// - Plays charging sound when used.
// - Allows swapping with non-powered items in inventory.
// - Fixes bug with passing invalid charge amount.
class scpBattery extends SqRootScript {
	function OnFrobToolEnd() {
		local obj = message().DstObjId;
		if (Property.Possessed(obj, "Energy") || (Property.Get(obj, "ShockWeaponType") == 1) || Object.InheritsFrom(obj, "Dead Power Cell") || Object.InheritsFrom(obj, "Power Cell")) {
			PostMessage(obj, "Recharge", 1000);
			stopTimer();
			SetData("CheckTimer", SetOneShotTimer("Check", 0.05));
			ShockGame.PreventSwap();
			ShockGame.RefreshInv();
		}
		else if (!scp.IsContained(obj)) {
			ShockGame.PreventSwap();
			scp.AddText("BatteryCant", "misc", "Item cannot be charged.");
		}
	}

	function OnFrobInvEnd() {
		scp.AddText("HelpBattery", "misc");
	}

	function OnConsume() {
		scp.Consume(self, "use_battery");
		stopTimer();
	}
	...
	function OnTimer() {
		if (message().name == "Check") {
			ClearData("CheckTimer");
			scp.AddText("BatteryMax", "misc", "Item already fully charged.");
		}
	}

	function stopTimer() {
		if (IsDataSet("CheckTimer")) {
			KillTimer(GetData("CheckTimer"));
			ClearData("CheckTimer");
		}
	}
}
...
class scpProxGrenade extends SqRootScript {
	...
	function OnPhysFellAsleep() {
		scp.CreateAt("Contacted Prox Grenade", self);
		Object.Destroy(self);
	}
}
...
class scpContactProxGrenade extends SqRootScript {
	...
	function OnInit() {
		...
		// create tripwire
		obj = scp.CreateAt("Prox Grenade Trigger", self);
...
class scpTeleportFlash extends SqRootScript {
	function OnTurnOn() {
		...
		scp.CreateAt("Citadel Teleport", "Player");
	}
}
...
class scpRecGrate extends SqRootScript {
	...
	function doCollapse() {
		...
		SetData("done", true);
		// spawn falling grate
		local obj = scp.CreateAt("Grate 6x8 falling", self);
	...
	function OnTurnOn() {
		...
		if (link) {
			...
			if (!Object.HasMetaProperty(DstObj, "Dematerialized")) {
				...
				Object.Teleport(DstObj, vector(), vector(), self);
				Sound.PlayEnvSchema(DstObj, "Event Activate", DstObj, 0, eEnvSoundLoc.kEnvSoundAtObjLoc);
...
class scpReaverFlash extends SqRootScript {
	function OnTurnOn() {
		ShockGame.StartFadeIn(2000, 128, 100, 90)
	}
}
...
class scpShodanShield extends SqRootScript {
	...
	function OnSlain() {
		if (!GetData("RetroMode")) {
			PostMessage("Big Shodan Head", "ShieldSlain");
		}
	}
...
// --------------------------------------------------------------------------------
// Handles disabling SHODAN shield interlock terminal by model swapping instead
// of corpsing so it doesn't flicker
class scpInterlockHelper extends SqRootScript {
	function OnBeginScript() {
		Property.SetSimple(self, "SlayResult", 1); // No Effect
	}

	function OnSlain() {
		Property.SetSimple(self, "ModelName", "shohackd");
		Object.AddMetaProperty(self, "Disable");
	}
}

// --------------------------------------------------------------------------------
// Handles shield interlocks being hacked in SHODAN boss fight
// Replaces script on HackMultiTrigger (276)
class scpShieldHackTrigger extends SqRootScript {
	function OnTurnOn() {
		local hacks = (GetData("HackCount") || 0) + 1;
		SetData("HackCount", hacks);
		if (hacks == 3) {
...
// - Performs various retro mode one-time setup tasks
// - Makes shots randomly emit from left or right eye
// - Shoots faster as more shield segments are destroyed
// - Spawns corpse when slain
class scpShodanHeadHelper extends SqRootScript {
	// 0: editor, 1: easy, 2: normal, 3: hard, 4: impossible, 5: multiplayer
	static diffhpMult = [1.5, 1, 1.3, 1.6, 2, 2];
	static diffShots  = [1, 0, 0.3, 0.6, 1, 1];
	...
	function OnBeginScript() {
		if (scp.IsModEnabled("modFinalBoss")) {
			...
			Property.SetSimple("CPU1", "HackText", "Shield_Interlock");
			Property.SetSimple("CPU2", "HackText", "Shield_Interlock");
			Property.SetSimple("CPU3", "HackText", "Shield_Interlock");
			return;
		}
		// init eye shot switching
		if (IsDataSet("EyeTimer")) {
			KillTimer(GetData("EyeTimer"));
		}
		SetData("EyeTimer", SetOneShotTimer("ICU", 1));
		// init difficulty-dependent health
		if (!IsDataSet("HpDone")) {
			SetData("HpDone", true);
			local hpNew = GetProperty("HitPoints") * diffhpMult[Quest.Get("Difficulty")];
			SetProperty("HitPoints", hpNew);
			SetProperty("MAX_HP", hpNew);
		}
	}
	...
	// sent by scpShodanShield
	function OnShieldSlain() {
		local lnk, obj, diffPer;
		local shields = 0;
		// count remaining shields
		foreach (lnk in Link.GetAll("SwitchLink", "HackMultiTrigger")) {
			obj = LinkDest(lnk);
			if (Object.InheritsFrom(obj, "ShodanShields") && Property.Get(obj, "HitPoints") > 0) {
				shields++;
			}
		}
		...
		diffPer = ((8 - shields) / 8.0) * diffShots[Quest.Get("Difficulty")];
		// make SHODAN fire faster as more shield segments are destroyed
		SetProperty("AI_Turret", "Fire Pause", 2000 - diffPer * 1750);
		SetProperty("AI_Turret", "Fire Epsilon", 0.1 + diffPer * 0.5);
	}
}

// --------------------------------------------------------------------------------
// Enhanced SHODAN death script
...
class scpTripSlay extends SqRootScript {
	...
	function OnPhysEnter() {
		...
		if (Object.InheritsFrom(obj, "Monsters")) {
			// must do this instead of Damage.Slay() to avoid a crash when killing anything that explodes into pieces
			ActReact.Stimulate(obj, ObjID("WeaponBash"), 1000, 0);
		}
...
// --------------------------------------------------------------------------------
// Sets Has Refs according to TurnOn/TurnOff messages
class scpRefsTrap extends SqRootScript {
	function OnTurnOn() {
		SetProperty("HasRefs", true);
	}
...
// - Fixes alarm reset only resetting camera belonging to ecology that started
//   the alarm, not any other cameras that spotted player during the alarm.
// - Fixes raise_spawn_rand allowing rand to go negative.
// - Consolidates TriggerEcology and TriggerEcologyDiff.
...
class TriggerEcology extends SqRootScript {
	function OnBeginScript() {
		...
		if (DEBUG_ECO_TYPES && Version.IsEditor()) {
			// validate EcoTypes
			local lnk, lnkD, spawnEco;
			local none = "NONE";
			local selfEco = HasProperty("EcoType") ? GetProperty("EcoType") : none;
			local header = (isAlertEco() ? "Alert " : "") + "Ecology " + self + " EcoType (" + selfEco + ") ";
			foreach (lnk in Link.GetAll("SwitchLink", self)) {
				lnkD = LinkDest(lnk);
				if (Object.InheritsFrom(lnkD, "Spawn")) {
					spawnEco = Property.PossessedSimple(lnkD, "EcoType") ? Property.Get(lnkD, "EcoType") : none;
					if (spawnEco != selfEco || selfEco == none || spawnEco == none) {
						Debug.MPrint("WARNING: " + header + "doesn't match Spawner " + lnkD + " EcoType (" + spawnEco + ")");
					}
					else {
						print(header + "matches Spawner " + lnkD + " EcoType (" + spawnEco + ")");
					}
				}
				else if (Object.InheritsFrom(lnkD, "QuestBits")) {
					Debug.MPrint("WARNING: " + header + "linked through QB filter; investigate manually");
				}
			}
		}
	}
	...
	function OnReset() {
		if (GetProperty("EcoState") == 2) {
			...
			// ensure all cameras are reset, not just this ecology's cameras
			local obj;
			foreach (obj in scp.GetConcretes("Security Camera")) {
				PostMessage(obj, "ResetEx");
			}
		}
	}
	...
	function EcoActivate() {
		...
		if (DEBUG_ECO_LEVEL && !(isAlertEco() && GetProperty("EcoState") == 0)) {
...
// --------------------------------------------------------------------------------
// Obsolete; functionality moved to TriggerEcology
class scpEcologyHelper extends SqRootScript {
	function OnBeginScript() {
		if (Version.IsEditor()) {
			print("scpEcologyHelper can be removed from " + self);
		}
	}
}
...
// Add "NoFarthest" in Design Note to override SPAWN_FORCE_FARTHEST.
...
class scpTrapSpawn extends SqRootScript {
	function OnBeginScript() {
		if (DEBUG_ECO_LEVEL == 2 && !IsDataSet("Init")) {
			SetData("Init", true);
			SetData("Spawns1", 0);
			SetData("Spawns2", 0);
			SetData("Spawns3", 0);
	...
	function findSpawnPoint(flags) {
		...
		else if ((eSpawnFlags.kSpawnFlagFarthest & flags) || (SPAWN_FORCE_FARTHEST && !("NoFarthest" in userparams()))) {
			// sort by distance
			dprint("Sorting by distance...", true);
			objList.sort(@(a,b) scp.DistSqFlat(Object.Position(objPlr), Object.Position(b)) <=> scp.DistSqFlat(Object.Position(objPlr), Object.Position(a)));
		}
		else {
		...
		if (DEBUG_ECO_LEVEL == 2) {
			foreach (obj in objList) {
				dprint("  " + obj + " (dist " + scp.DistSqFlat(Object.Position(objPlr), Object.Position(obj)) + ")");
			}
		}
	...
	function validSpawn(obj, flags) {
		...
		if ((eSpawnFlags.kSpawnFlagPlayerDist & flags) && (scp.DistSqFlat(Object.Position("Player"), Object.Position(obj)) < 900)) {
	...
	function dprint(txt, verbose = false) {
		if ((DEBUG_ECO_LEVEL == 1 && !verbose) || (DEBUG_ECO_LEVEL == 2)) {
			scp.Trace(txt);
		}
	}
}
...
class scpRadiusDamageSponge extends SqRootScript {
	...
	function OnSimDmg() {
		local m = message();
		// make sure emough time has passed that this damage must be
		// from a different source; prevents target taking double-dip
		// damage from same projectile
...
class scpTripwireDebug extends SqRootScript {
	function OnBeginScript() {
		if (!Version.IsEditor() || !DEBUG_TRIPWIRES) {
			return;
		}
...
// --------------------------------------------------------------------------------
// Displays entity damage info in mono
...
class scp extends SqRootScript {
	// map encoding table
	static maps = ["command1", "command2", "earth", "eng1", "eng2", "hydro1", "hydro2",
		"hydro3", "many", "medsci1", "medsci2", "ops1", "ops2", "ops3", "ops4", "rec1",
		"rec2", "rec3", "rick1", "rick2", "rick3", "shodan", "station"];
	...
	// - values to be substituted for %d or %s in string (optional)
	...
	function Consume(obj, schema = null) {
		local count = 0;
	...
	// determine if object is hackable
	function IsHackable(obj) {
		return Property.Possessed(obj, "HackDiff") &&
			Property.Get(obj, "HackDiff", "Success %") > -1000 &&
			!(Property.Possessed(obj, "HitPoints") && Property.Get(obj, "HitPoints") < 1) &&
			Property.Get(obj, "ObjState") != eObjState.kObjStateBroken &&
			Property.Get(obj, "ObjState") != eObjState.kObjStateHacked;
	}

	// determine if object is contained in another object's inventory
	function IsContained(obj) {
		return Link.AnyExist("~Contains", obj);
	}
	...
	function SilenceEcoXerxes() {
		local eco;
		foreach (eco in scp.GetConcretes("Ecology")) {
			Sound.HaltSchema(eco);
		}
	}
	...
	// safely spawn a new object at the specified location
	function CreateAt(archetype, target) {
		local obj = Object.BeginCreate(archetype);
		Object.Teleport(obj, vector(), vector(), target);
		Object.EndCreate(obj);
		return obj;
	}

	// close all stat upgrade overlays
	function UpgradeCheatStopper() {
		ShockGame.OverlayChange(kOverlayBuyStats, kOverlayModeOff);
		ShockGame.OverlayChange(kOverlayBuyTech, kOverlayModeOff);
		ShockGame.OverlayChange(kOverlayBuyWeapon, kOverlayModeOff);
		ShockGame.OverlayChange(kOverlayBuyPsi, kOverlayModeOff);
	}
	...
	// encode map name into an ID number
	function EncMapName(mapName) {
		local mapID = maps.find(mapName);
		if (mapID == null) {
			mapID = -1;
			scp.Trace("ERROR: Unrecognized map name");
		}
		return mapID + 1;
	}

	// decode map ID into a string
	function DecMapName(mapID) {
		return mapID.tointeger() == 0 ? "" : maps[mapID - 1];
	}
	...
	// return list of all concretes that descend from requested archetype
	// - accepts optional filter function
	// - accepts optional argument for filter function
	// - must use argument to pass in "self" if filter uses it, since lib
	//   functions have no sense of self
	...
	// return distance squared between two points on X/Y plane
	function DistSqFlat(v1, v2) {
